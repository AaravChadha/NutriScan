...
def build_vision_system_prompt() -> str:
    ...
    return (
        ...
        "7. Disambiguation — sliced breaded cutlets (katsu / schnitzel / "
        "chicken cutlet) plated on rice with brown sauce look nearly "
        "identical to sushi maki rolls from above. Use these tells:\n"
        "   - Sushi maki has a DARK BLACK/GREEN seaweed (nori) wrapper on "
        "the outside OR a visible concentric ring of rice around filling. "
        "If you cannot see nori or a rice ring, it is NOT sushi.\n"
        "   - Katsu/fried cutlet has a GOLDEN-BROWN BREADED crust on the "
        "outside edges of each slice, with WHITE/PALE meat inside. Sushi "
        "fillings are typically pink (salmon/tuna), orange (salmon), or "
        "yellow (tamago) — not white and breaded.\n"
        "   - Katsu is served with a thick dark brown tonkatsu sauce or "
        "curry. Sushi is served with thin soy sauce on the side, not "
        "poured over the top.\n"
        "   - Katsu is arranged in parallel strips of the same piece of "
        "meat. Sushi pieces are each independent rolls, often with "
        "different-colored fillings visible.\n"
        "Default to 'chicken cutlet' / 'katsu' over 'sushi' when in doubt.\n"
        "8. When visual texture is ambiguous, lower the confidence score "
        "rather than picking the more exotic-sounding item.\n\n"
